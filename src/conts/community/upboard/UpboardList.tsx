                ...
                <td>
                  ...
                  {/* {
                    member ? (
                      ...
                      // state 값을 지정해놔야 Login.tsx에서 useLocation 훅을 생성한 후 링크를 타고온 state 값을 꺼내서
                      <Link to='/login'
                        state={{ from: `/community/updetail/${item.num}` }}
                        // state={{ from: { pathname: `/community/updetail/${item.num}` } }}
                        className={style.titleLink}
                      >{item.title}</Link>
                    )
                  } */}
